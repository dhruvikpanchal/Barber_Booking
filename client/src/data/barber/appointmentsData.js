...
import { DEMO_CUSTOMER } from "@/data/customer/appointmentsData.js";
...
export function normalizeBarberAppointment(appt) {
  ...
  const price =
    appt.price ?? services.reduce((sum, s) => sum + (s.price ?? 0), 0);
  const duration =
    appt.duration ?? services.reduce((sum, s) => sum + (s.duration ?? 0), 0);
