...
import { ANALYTICS_PERIODS } from "@/constants/barber/analytics.js";
  ...
  return (
    <>
          ...
          {ANALYTICS_PERIODS.map((p) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() =>
                  p.key === "custom" ? openCustom() : onPeriodChange(p.key)
                }
      ...
      {modalOpen ? (
        ...
        >
          <div className="w-full max-w-md space-y-4 rounded-xl border border-outline-variant bg-surface-container p-5 shadow-2xl">
            <div className="flex items-start justify-between gap-3">
              ...
              <button
                ...
                aria-label="Close"
                className="flex h-8 w-8 items-center justify-center rounded-md text-on-surface-variant hover:bg-surface-container-high"
              >
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="block text-sm">
                ...
                <input
                  ...
                  className="mt-1 h-10 w-full rounded-md border border-outline-variant bg-surface-container-low px-3 text-sm text-on-surface focus:border-primary focus:outline-none"
                />
              </label>
              <label className="block text-sm">
                <span className="font-label-caps text-on-surface-variant">
            ...
            <div className="flex gap-3">
              <button
                type="button"
                onClick={() => setModalOpen(false)}
                className="flex-1 rounded-md border border-outline-variant py-2.5 text-sm font-medium text-on-surface-variant hover:bg-surface-container-high"
              >
                Cancel
              </button>
              <button
                type="button"
                onClick={applyCustom}
                className="flex-1 rounded-md bg-primary py-2.5 text-sm font-bold text-on-primary hover:opacity-90"
              >
                Apply range
              </button>
            </div>
