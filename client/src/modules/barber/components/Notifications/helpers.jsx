...
export function Avatar({ initials, size = "md" }) {
  ...
  return (
    <div
      className={`${sz} rounded-full bg-primary/15 border border-primary/25 flex items-center justify-center
        font-semibold text-primary shrink-0 font-sans`}
    >
      {initials}
    </div>
  );
  ...
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => (
        <Star
          key={i}
          className={`w-3.5 h-3.5 ${i <= rating ? "fill-yellow-400 text-yellow-400" : "text-outline-variant"}`}
        />
      ))}
    </div>
  );
...
export function ActionButtons({ id, type, onAction }) {
  const [state, setState] = useState("idle"); // idle | loading-accept | loading-decline | accepted | declined
