...
export function ReplyModal({ review, onClose, onSubmit }) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-lg space-y-4 rounded-xl border border-outline-variant bg-surface-container p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-0.5 text-sm text-on-surface-variant">
              {review.customer.name} · {formatDate(review.date)}
            </p>
          </div>
          <button
            type="button"
