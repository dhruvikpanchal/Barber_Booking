...
import {
  ...
  EXP_TIERS,
  SPECIALTIES,
} from "@/constants/barber/barber.js";
import { Field, IconInput } from "./components/Profile/helpers.jsx";
import PublicPreview from "./components/Profile/PublicPreview.jsx";
import SectionCard from "./components/Profile/SectionCard.jsx";
import { INITIAL_PROFILE } from "../../data/barber/profileData.js";
...
export default function BarberProfile() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 pb-28 md:space-y-8 md:pb-8">
      <header className="space-y-4">
        <div className="space-y-2">
          ...
          <p className="max-w-2xl text-sm leading-relaxed text-on-surface-variant">
            ...
            Clients see the preview when browsing and booking.
          </p>
        </div>

        <nav
          aria-label="Profile sections"
          className="-mx-1 flex gap-1 overflow-x-auto pb-1 scrollbar-thin md:hidden"
        >
          {sections.map((s) => (
            <button
              key={s.id}
          ...
          >
            <div className="space-y-5">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
                <div className="relative mx-auto h-28 w-28 shrink-0 overflow-hidden rounded-xl border border-outline-variant bg-surface-container sm:mx-0">
                  ...
                  <label className="absolute inset-0 flex cursor-pointer items-center justify-center bg-black/50 opacity-0 transition-opacity hover:opacity-100">
                    <Camera className="h-6 w-6 text-on-primary" aria-hidden />
                    <input
                      type="file"
                      accept="image/*"
                      className="sr-only"
                ...
                <div className="flex-1 space-y-4">
                  ...
                  <div className="flex items-center gap-3 rounded-lg border border-outline-variant bg-surface-container px-4 py-3">
                    <button
                      type="button"
                      role="switch"
                      aria-checked={profile.availableToday}
                      onClick={() =>
                        patch({ availableToday: !profile.availableToday })
                      }
              ...
              <Field
                label="Short bio"
                hint={`${profile.bio.length}/200 characters`}
              >
                <textarea
                  maxLength={200}
                  rows={3}
                ...
                >
                  <div className="relative aspect-square">
                    ...
                    <button
                      ...
                      className="absolute right-1.5 top-1.5 inline-flex h-7 w-7 items-center justify-center rounded-md bg-black/60 text-on-primary opacity-100 transition-opacity sm:opacity-0 sm:group-hover:opacity-100"
                    >
                      <X className="h-4 w-4" />
                    </button>
                  </div>
                  <input
                    ...
                    placeholder="Caption"
                    className="w-full border-t border-outline-variant bg-transparent px-2 py-1.5 text-xs text-on-surface placeholder:text-on-surface-variant/50 focus:outline-none"
                  />
                </li>
              ))}
              {profile.portfolio.length < 8 ? (
                <li>
                  <label className="flex aspect-square cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-outline-variant bg-surface-container text-on-surface-variant transition-colors hover:border-primary hover:text-primary">
          ...
          >
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                <Field
                  label="Instagram"
                  hint="Optional — shown on your public card."
                >
                  <input
                    type="text"
        ...
        <aside id="preview" className="scroll-mt-24 lg:sticky lg:top-4">
          ...
          >
            ...
            <p className="mt-3 text-[11px] leading-relaxed text-on-surface-variant">
              Star rating and review count are demo values until reviews sync
              from the platform.
            </p>
          </SectionCard>
        </aside>
      ...
      <div className="fixed inset-x-0 bottom-[var(--bottom-nav-height)] z-30 border-t border-outline-variant bg-surface/95 px-4 py-3 backdrop-blur md:static md:bottom-auto md:z-auto md:border-t md:bg-transparent md:px-0 md:py-0 md:pt-6">
        <div className="mx-auto flex max-w-6xl flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-center text-xs text-on-surface-variant sm:text-left">
            {saved
              ? "Profile saved."
              : "Changes are local until backend is connected."}
          </p>
          <button
            type="button"
            onClick={handleSave}
            className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-md bg-primary text-sm font-semibold text-on-primary transition-all hover:opacity-90 active:scale-[0.98] sm:w-auto sm:px-8"
          >
