...
import WalkInFormModal from "./components/WalkIns/WalkInFormModal";
import QueueStats from "./components/WalkIns/QueueStats";
import {
  STATUS_ORDER,
  STATUSES,
  WALK_IN_TABS,
} from "../../constants/barber/walk_In";
import { EMPTY_WALK_IN_FORM } from "@/constants/shared/empty_form.js";
import { INITIAL } from "../../data/barber/walkinsData.js";
...
export default function WalkIns() {
  ...
  const filtered = useMemo(() => {
    ...
    if (tab === "active") {
      list = entries.filter(
        (e) => e.status === "waiting" || e.status === "in-service",
      );
    } else if (tab !== "all") {
  ...
  const waitingOrder = useMemo(() => {
    ...
    entries
      .filter((e) => e.status === "in-service")
      .forEach((e) => map.set(e.id, "•"));
    return map;
  }, [entries]);
  ...
  function reopen(id) {
    setEntries((prev) =>
      prev.map((e) =>
        e.id === id ? { ...e, status: "waiting", addedAt: Date.now() } : e,
      ),
    );
  }
  function remove(id) {
      ...
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-2">
          ...
          <p className="max-w-xl text-sm leading-relaxed text-on-surface-variant">
            Drop in customers, manage the live queue, and move them through
            service with one tap.
          </p>
        </div>
        <button
          type="button"
