...
import ChairBoard from "./components/Queue/ChairBoard";
import QueueRow from "./components/Queue/QueueRow";
import AddToQueueModal, {
  EMPTY_FORM,
} from "./components/Queue/AddToQueueModal";
import { INITIAL_CHAIRS, INITIAL_QUEUE } from "../../data/barber/queueData";
import { STATUS_ORDER } from "../../constants/barber/queue";
import { QUEUE_TABS, QUEUE_SOURCE_TABS } from "@/constants/barber/barber.js";
...
export default function Queue() {
  ...
  const chairForCustomer = (customerId) =>
    chairs.find((c) => c.customerId === customerId);
  ...
  function seatAtChair(chairId, customerId) {
    ...
    setQueue((prev) =>
      prev.map((q) =>
        q.id === customerId
          ? { ...q, status: "in-service", chairId, startedAt: Date.now() }
          : q,
      ),
    );
  }
  ...
  function completeByChair(chairId) {
    ...
    setChairs((prev) =>
      prev.map((c) => (c.id === chairId ? { ...c, customerId: null } : c)),
    );
    setQueue((prev) =>
      prev.map((q) =>
        q.id === customerId
          ? { ...q, status: "done", completedAt: Date.now(), chairId: null }
          : q,
      ),
    );
  }
  ...
  function completeByCustomer(customerId) {
    ...
    setQueue((prev) =>
      prev.map((q) =>
        q.id === customerId
          ? { ...q, status: "done", completedAt: Date.now() }
          : q,
      ),
    );
  }
  ...
  function cancelCustomer(id) {
    ...
    if (chair) {
      setChairs((prev) =>
        prev.map((c) => (c.id === chair.id ? { ...c, customerId: null } : c)),
      );
    }
    setQueue((prev) =>
      prev.map((q) =>
        q.id === id ? { ...q, status: "cancelled", chairId: null } : q,
      ),
    );
  }
  ...
  function reopen(id) {
    setQueue((prev) =>
      prev.map((q) =>
        q.id === id
          ? { ...q, status: "waiting", chairId: null, addedAt: Date.now() }
          : q,
      ),
    );
  }
    ...
    <div className="mx-auto max-w-6xl space-y-8 pb-6">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-2">
          ...
          <p className="flex max-w-xl items-center gap-2 text-sm leading-relaxed text-on-surface-variant">
            <Radio
              className="h-3.5 w-3.5 animate-pulse text-status-confirmed"
              aria-hidden
            />
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <p className="text-xs text-on-surface-variant">
            {stats.chairsFree} free · {chairs.length - stats.chairsFree}{" "}
            occupied
          </p>
        </div>
        <ChairBoard
      ...
      <section className="rounded-xl border border-outline-variant bg-surface-container-low">
        <div className="flex flex-col gap-4 border-b border-outline-variant px-5 py-4 md:px-6">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div className="scrollbar-thin -mx-1 flex gap-1 overflow-x-auto rounded-lg border border-outline-variant p-0.5">
              {QUEUE_TABS.map((t) => {
        ...
        <div className="space-y-3 p-4 md:p-6">
          {filtered.length > 0 ? (
            filtered.map((entry) => {
              const waitIdx = waitingOrdered.findIndex(
                (q) => q.id === entry.id,
              );
              ...
              return (
                <QueueRow
                  ...
                  canMoveUp={waitIdx > 0}
                  canMoveDown={
                    waitIdx >= 0 && waitIdx < waitingOrdered.length - 1
                  }
                  onMove={moveInQueue}
                  onSeat={seatSpecific}
            ...
            <div className="rounded-lg border border-dashed border-outline-variant px-4 py-14 text-center">
