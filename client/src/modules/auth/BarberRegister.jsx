...
import { getPasswordStrength, PASSWORD_STRENGTH } from "@/constants/shared/password.js";
import { SPECIALTIES, EXP_TIERS } from "@/constants/barber/barber.js";
...
function Input({ className = "", ...props }) {
  return (
    <input
      {...props}
      className={`w-full border border-[#3a2e28] bg-[#1c1c1c] px-4 py-3 text-sm text-[#e4e2e1] transition-colors duration-200 placeholder:text-[#d8c2b7]/30 focus:border-[#ffb68c] focus:bg-[#1f1a17] focus:outline-none ${className}`}
    />
  );
}
...
function Select({ className = "", children, ...props }) {
  return (
    <select
      {...props}
      className={`w-full cursor-pointer appearance-none border border-[#3a2e28] bg-[#1c1c1c] px-4 py-3 text-sm text-[#e4e2e1] transition-colors duration-200 focus:border-[#ffb68c] focus:outline-none ${className}`}
    >
      {children}
    </select>
  );
  ...
  return (
    <div className="space-y-5">
      ...
      <Field label="PROFILE PHOTO" hint="Square image recommended. JPG, PNG or WEBP · max 4 MB.">
        <label className="group flex cursor-pointer items-center gap-4 border border-dashed border-[#3a2e28] px-4 py-4 transition-colors hover:border-[#ffb68c]/60">
          ...
          <input
            ...
            accept="image/*"
            className="sr-only"
            onChange={(e) => setData({ ...data, photoName: e.target.files?.[0]?.name ?? "" })}
          />
        </label>
      </Field>
    </div>
...
function StepProfessional({ data, setData }) {
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {EXP_TIERS.map((tier) => (
            <button
              ...
              className={`border px-2 py-3 text-center transition-all duration-200 active:scale-95 ${
      ...
      <Field label="SHORT BIO" hint="Shown on your public barber profile. 200 chars max.">
        <textarea
          ...
          className="w-full resize-none border border-[#3a2e28] bg-[#1c1c1c] px-4 py-3 text-sm text-[#e4e2e1] transition-colors placeholder:text-[#d8c2b7]/30 focus:border-[#ffb68c] focus:outline-none"
        />
          ...
          {SPECIALTIES.map((s) => {
            ...
            return (
              <button
                ...
                className={`border px-3 py-1.5 text-[11px] tracking-wide transition-all duration-150 active:scale-95 ${
                  active
...
/* ─── Step 3: Security ─── */
function StepSecurity({ data, setData }) {
      ...
      <Field label="PASSWORD">
        <div className="relative">
          <input
            ...
            className="w-full border border-[#3a2e28] bg-[#1c1c1c] px-4 py-3 pr-12 text-sm text-[#e4e2e1] transition-colors placeholder:text-[#d8c2b7]/30 focus:border-[#ffb68c] focus:outline-none"
          />
          <button
            type="button"
      ...
      <div className="space-y-3 pt-2">
        ...
        <label className="group flex cursor-pointer items-start gap-3">
          <div
            ...
            onClick={() => setData({ ...data, barberAgreement: !data.barberAgreement })}
          >
      ...
      <section className="flex min-h-screen items-center justify-center bg-[#131313] px-6 text-[#e4e2e1]">
        <div className="w-full max-w-md text-center">
          ...
          <p className="mb-10 text-sm leading-relaxed text-[#d8c2b7]">
            Our admin team will review your profile and send a confirmation email within 1–2
            business days. Once approved, your barber dashboard will unlock automatically.
          </p>
      ...
      <div className="flex w-full flex-col justify-center overflow-y-auto px-6 py-14 md:px-14 lg:w-[58%] lg:px-16 xl:w-[62%] xl:px-24">
        <div className="mx-auto w-full max-w-lg">
          ...
          <form onSubmit={handleSubmit}>
            {step === 1 && <StepPersonal data={personal} setData={setPersonal} />}
            {step === 2 && <StepProfessional data={professional} setData={setProfessional} />}
            {step === 3 && <StepSecurity data={security} setData={setSecurity} />}
            ...
            <div className={`mt-8 flex gap-4 ${step > 1 ? "justify-between" : "justify-end"}`}>
              {step > 1 && (
                <button
                  type="button"
                  onClick={handleBack}
                  className="flex items-center gap-2 border border-[#3a2e28] px-6 py-3 text-xs tracking-[0.15em] text-[#d8c2b7] transition-all hover:border-[#a08d83] hover:text-[#e4e2e1] active:scale-95"
                >
                  <ChevronLeft size={14} />
                  BACK
                </button>
              ...
              {step < 3 ? (
                <button
                  type="button"
                  onClick={handleNext}
                  className="ml-auto flex items-center gap-2 bg-[#ffb68c] px-8 py-3 text-xs font-semibold tracking-[0.2em] text-[#532200] transition-all hover:opacity-90 active:scale-95"
                >
                  NEXT
          ...
          <div className="mt-10 flex flex-wrap items-center justify-between gap-4 border-t border-[#2a2017] pt-6">
            <p className="text-xs text-[#a08d83]">
              Already registered?{" "}
              <Link href="/login" className="text-[#ffb68c] transition hover:opacity-70">
                SIGN IN
              </Link>
            </p>
            <Link
              href="/barber-register-rules"
