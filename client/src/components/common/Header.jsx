    ...
    <>
      ...
      >
        ...
        <nav className="hidden lg:flex gap-6 items-center" aria-label="Main navigation">
          ...
          {['Services', 'Barbers', 'Gallery', 'About', 'Contact'].map((item) => {
      ...
      {/* Mobile Nav */}
      <div className={`fixed inset-0 z-[49] ${mobileMenu ? 'block' : 'hidden'}`}>
        {/* Backdrop */}
        <div onClick={() => setMobileMenu(false)} className="absolute inset-0 bg-black/70"></div>

        {/* Sidebar */}
        <nav className="absolute right-0 top-12 h-full w-72 bg-[#1b1c1c] border-l border-[#53443c] flex flex-col p-8 gap-5 overflow-y-auto">
          {/* Mobile Links */}
          <div className="flex flex-col gap-6 ">
            <Link href="/" className="text-[#ffb68c] text-xs font-semibold tracking-[0.1em]">
              HOME
            </Link>
          ...
          {/* Bottom Buttons */}
          <div className="mt-10 flex flex-col gap-2 border-t border-[#53443c] pt-8">
            <Link
              href="/login"
