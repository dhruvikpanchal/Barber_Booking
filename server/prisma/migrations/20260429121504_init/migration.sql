...
CREATE TABLE `users` (
    ...
    `reset_password_otp` VARCHAR(191) NULL,
    ...
    INDEX `users_email_provider_idx`(`email`, `provider`),
    PRIMARY KEY (`id`)
...
-- AddForeignKey
ALTER TABLE `Booking` ADD CONSTRAINT `Booking_barber_id_fkey` FOREIGN KEY (`barber_id`) REFERENCES `barbers`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `Booking` ADD CONSTRAINT `Booking_slot_id_fkey` FOREIGN KEY (`slot_id`) REFERENCES `time_slots`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
