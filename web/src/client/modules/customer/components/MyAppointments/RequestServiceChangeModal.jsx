...
import { servicesEqual } from "@/client/modules/customer/helpers/appointmentsHelpers.js";
...
export default function RequestServiceChangeModal({
  ...
  const addOptions = useMemo(
    () =>
      catalog.filter((c) => !selected.some((s) => s.id === c.id || (!s.id && s.name === c.name))),
    [selected, catalog],
  );
  ...
  function replaceService(oldKey, newId) {
    ...
    if (!svc) return;
    setSelected((prev) => prev.map((s) => ((s.id ?? s.name) === oldKey ? { ...svc } : s)));
    setError("");
  }
        ...
        <div className="scrollbar-thin min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4">
          {selected.map((s) => {
            ...
            return (
              ...
              >
                <span className="text-on-surface min-w-0 flex-1 text-sm font-medium">{s.name}</span>
                <select
                  value=""
