...
import { GENDERS, DEFAULT_HOURS } from "@/modules/admin/constants/barberDetailConstants.js";
  ...
  const counts = weights.map((w, i) => {
    ...
    if (star === Math.round(rating))
      return (
        ...
        Math.max(
          0,
          reviewCount - weights.reduce((s, _, j) => s + Math.round(reviewCount * weights[j]), 0),
        )
      );
    return base;
  });
...
function buildRecentReviews(barber) {
  ...
  return samples.map((s, i) => ({
    ...
    text: s.text,
    rating: s.rating,
    date: new Date(Date.now() - s.daysAgo * 24 * 60 * 60 * 1000).toISOString(),
  }));
}
