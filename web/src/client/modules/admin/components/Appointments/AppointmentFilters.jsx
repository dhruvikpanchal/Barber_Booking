  ...
  return (
    <div className="border-outline-variant space-y-4 border-b px-4 py-4 md:px-6">
      <div className="flex flex-wrap items-center gap-2">
        <span className="font-label-caps text-on-surface-variant inline-flex items-center gap-1.5 text-xs">
          <Filter className="h-3.5 w-3.5" aria-hidden />
          Status
        </span>
        {APPOINTMENT_STATUS_FILTER_ORDER.map((key) => {
          const active = status === key;
          const label = key === "all" ? "All" : (APPOINTMENT_STATUSES[key]?.label ?? key);
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:gap-4">
        ...
        <label className="relative block min-w-0 flex-1 lg:max-w-xs">
          <span className="font-label-caps text-on-surface-variant mb-1.5 block text-[10px]">
            Search
          </span>
          <Search
            className="text-on-surface-variant pointer-events-none absolute bottom-2.5 left-3 h-4 w-4"
            aria-hidden
          />
          <input
            value={query}
