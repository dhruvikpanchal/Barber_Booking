...
export default function ServiceUpdatedModal({ appt, onClose }) {
  ...
  const priceNote =
    appt.originalPrice != null
      ? `Price was ${formatMoney(appt.originalPrice)}, now ${formatMoney(appt.price)}`
      : `Current price ${formatMoney(appt.price)}`;
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      size="md"
      labelledBy="service-updated-title"
      panelClassName="rounded-xl border border-outline-variant bg-surface-container-low shadow-2xl"
    >
          ...
          <>
            <div className="flex items-center gap-3 rounded-lg border border-outline-variant bg-surface-container p-4">
              <div className="min-w-0 flex-1">
                <p className="font-label-caps text-[10px] text-on-surface-variant">Previous</p>
              ...
              <div className="min-w-0 flex-1 text-right">
                <p className="font-label-caps text-[10px] text-on-surface-variant">Current</p>
                <p className="truncate text-sm font-semibold text-primary">{appt.service}</p>
              </div>
            </div>
