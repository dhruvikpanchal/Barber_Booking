...
import { INITIAL_BARBERS } from "@/modules/admin/data/barberData.js";
import { INITIAL_USERS } from "@/modules/admin/data/usersData.js";
import { INITIAL_ADMIN_APPOINTMENTS } from "@/modules/admin/data/appointmentsData.js";
...
function resolveServiceMeta(serviceName) {
  ...
  const match =
    SERVICE_CATALOG.find((s) => normalized.includes(s.name.toLowerCase().split(" ")[0])) ??
    SERVICE_CATALOG[0];
...
function buildStatusHistory(appt) {
  ...
  return steps.map((step) => {
    ...
    else if (current === "cancelled" && step.key === "pending") state = "complete";
...
function buildTimeline(appt) {
  ...
  if (appt.status !== "cancelled" && appt.status !== "pending") {
    events.push({
      id: `${appt.id}-payment`,
      type: "payment",
      title: appt.status === "completed" ? "Payment completed" : "Payment pending",
...
function resolveAdminBarberId(barberId, barberName) {
  const bySlug = INITIAL_BARBERS.find((b) => b.name.toLowerCase() === barberName.toLowerCase());
