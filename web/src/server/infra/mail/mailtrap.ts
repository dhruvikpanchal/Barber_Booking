...
export async function sendPasswordReset(
  to: string,
  token: string,
  resetFlowToken: string,
): Promise<void> {
  const verifyLink = appUrl(
    `/verify-otp?flow=${encodeURIComponent(resetFlowToken)}&email=${encodeURIComponent(to)}`,
  );
...
export async function sendBookingCancellation(
  opts: BookingEmailOpts & { reason?: string },
): Promise<void> {
  const when = opts.startAt.toLocaleString();
  await sendMail({
    to: opts.to,
